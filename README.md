...
- an optional `hatchkit_insert_input_instance` MCP tool that resolves an exact Ready State × Content Variant, preserves governed Label, Text, and Supporting text properties, inserts one real Instance, and fails before dispatch while the public Input fixture remains `unbuilt`;
