...
| 事实                                              | 权威来源           | 其他地方只保存什么                    |
| ------------------------------------------------- | ------------------ | ------------------------------------- |
| Brief、Token、Component Contract、Approval        | Git Catalog        | Agent Context 和 Figma 只是展示／投影 |
...
| `figma-plugin` | Plugin UI、Bridge Client、Figma Adapter、Variables／Button／Instance Writer 和三类审计                  | Figma Sandbox + Plugin iframe；只依赖 `core`   |
...
Core 的版本化 Writer Protocol 定义七种命令：
...
因此，当前 Agent 公开黄金路径是“查询已 Ready Button → 精确解析 Variant → 插入 Instance → 三类审计”。Variables 和 Button 建库链路已有协议、Plan、Plugin Writer、幂等与审计，但尚未作为公开 MCP Tool 交给 Agent 自主调用。
...
- Variables、Button Component Set 与 Button Instance 的确定性 Writer 及幂等恢复；
...
- Icon、Input 和三套 UI Direction 评审链路；
