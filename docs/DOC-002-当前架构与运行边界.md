...
| `figma-plugin` | Plugin UI、Bridge Client、Figma Adapter、Variables／Button／Icon／Input／官方平台 Instance Writer 和四类审计 | Figma Sandbox + Plugin iframe；只依赖 `core`   |
...
Writer-enabled MCP 在默认七个只读 Tool 之外增加：

- `hatchkit_ensure_variables`；
- `hatchkit_ensure_component`；
- `hatchkit_insert_button_instance`；
- `hatchkit_insert_icon_instance`；
- `hatchkit_insert_input_instance`；
- `hatchkit_insert_platform_instance`；
- `hatchkit_audit_styles`；
- `hatchkit_audit_components`；
- `hatchkit_audit_registry_drift`；
- `hatchkit_audit_platform_components`。

其中四个 Audit Tool 经过 Bridge／Plugin 读取 Figma，但协议上是 `read_only_diagnostic`，不修改文件。

Bridge 只绑定 HTTP `127.0.0.1:38451`；Session Token 由 Bridge 启动时生成，默认八小时失效，断开或停止 Bridge 后立即失效。

## 5. Writer Protocol 当前能表达什么

Core 的版本化 Writer Protocol 定义十三种命令：

| Command                     | 性质                                | 当前入口                          |
| --------------------------- | ----------------------------------- | --------------------------------- |
| `writer.ping`               | 只读连接诊断                        | Bridge／Plugin                    |
| `variables.ensure`          | 幂等创建／收敛 Variables            | `hatchkit_ensure_variables`       |
| `components.button.ensure`  | 幂等创建／收敛 Button Component Set | `hatchkit_ensure_component`       |
| `components.icon.ensure`    | 幂等创建／收敛 Icon Component Set   | `hatchkit_ensure_component`       |
| `components.input.ensure`   | 幂等创建／收敛 Input Component Set  | `hatchkit_ensure_component`       |
| `instances.button.insert`   | 幂等插入真实 Button Instance        | `hatchkit_insert_button_instance` |
| `instances.icon.insert`     | 幂等插入真实 Icon Instance          | `hatchkit_insert_icon_instance`   |
| `instances.input.insert`    | 幂等插入真实 Input Instance         | `hatchkit_insert_input_instance`  |
| `instances.platform.insert` | 幂等插入官方远程平台 Instance       | `hatchkit_insert_platform_instance` |
...
| `audit.platform-components.scan` | 当前页官方平台组件来源审计     | `hatchkit_audit_platform_components` |

因此，当前 Agent 公开黄金路径覆盖“确保 Variables → 确保 Component Set → 精确解析 Variant → 插入 Instance → 审计”。`hatchkit_ensure_component` 根据准确 Contract Profile自动路由 Button、Icon 或 Input，不接受 Agent 注入 Approval、摘要、Node ID 或 File Binding；这些安全事实全部从当前 Git Catalog 与 Registry 重建。官方平台路径另按准确 Platform Target 与 Platform Binding 解析 Apple／Google 发布 Component Key，只写入 `design-page` 文件并保持远程 Instance。公开 Icon 与 Input Registry 仍诚实标为 `unbuilt`，官方真实 Key 也仍需人工 Figma 验证；缺少真实 Approval Record 时会由 Bridge 在 Queue 前失败关闭。

## 6. Component Instance 端到端流程

1. Agent 只提交页面意图：准确 Component ID、Variant（Button 含 Label，Icon 含 Size，Input 含 State、Content 与三段可见文本）、坐标和稳定 `requestId`；
...
8. Plugin 创建或返回同一稳定 Instance，设置该组件允许的治理属性和坐标；
...
- Variables、Button／Icon／Input Component Set 与三类真实 Instance 的确定性 Writer 及幂等恢复；
- Approval 状态的 Queue 前失败关闭；
- 样式、组件来源和 Registry 双向漂移审计；
- Platform Target、官方外部 Registry、精确解析、远程 Instance Writer 与平台来源审计的本地模拟回归；
- Agent 黄金路径与系统失败矩阵。
...
- Icon 与 Input 的真实审批、Figma Desktop 建库与 Instance 插入验收；
- Apple iOS／iPadOS 26 Stable、27 Preview 与 Google Android／Material 3 的真实 Library Key、Component Key、Property Mapping 和远程 Instance 验收；
