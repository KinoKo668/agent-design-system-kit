...
| `components.input.ensure`   | 幂等创建／收敛 Input Component Set  | 底层 Writer 已实现，尚无面向 Agent 的公开 MCP Tool |
...
因此，当前 Agent 公开黄金路径是“查询已 Ready 组件 → 精确解析 Variant → 插入 Instance → 三类审计”。Button 可直接走完样例链路；Icon 与 Input 具备同等级的插入协议、Plan、Plugin Writer 与 MCP Tool，但公开 Registry 仍诚实标为 `unbuilt`，调用会在 Writer 派发前失败关闭。Variables、Button、Icon 与 Input 建库链路已有协议、Plan、Plugin Writer、幂等与 Registry 最终化，但尚未作为公开 MCP Tool 交给 Agent 自主调用。
