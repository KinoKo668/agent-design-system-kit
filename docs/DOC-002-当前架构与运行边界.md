...
| `figma-plugin` | Plugin UI、Bridge Client、Figma Adapter、Variables／Button／Icon／Input／Instance Writer 和三类审计     | Figma Sandbox + Plugin iframe；只依赖 `core`   |
...
Core 的版本化 Writer Protocol 定义十一种命令：
...
因此，当前 Agent 公开黄金路径覆盖“确保 Variables → 确保 Component Set → 精确解析 Variant → 插入 Instance → 三类审计”。`hatchkit_ensure_component` 根据准确 Contract Profile 自动路由 Button、Icon 或 Input，不接受 Agent 注入 Approval、摘要、Node ID 或 File Binding；这些安全事实全部从当前 Git Catalog 与 Registry 重建。公开 Icon 与 Input Registry 仍诚实标为 `unbuilt`，缺少真实 Approval Record 时会由 Bridge 在 Queue 前失败关闭。
