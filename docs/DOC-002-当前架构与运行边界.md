...
Core 的版本化 Writer Protocol 定义九种命令：
...
因此，当前 Agent 公开黄金路径是“查询已 Ready 组件 → 精确解析 Variant → 插入 Instance → 三类审计”。Button 可直接走完样例链路；Icon 具备同等级的插入协议、Plan、Plugin Writer 与 MCP Tool，但公开 Icon Registry 仍诚实标为 `unbuilt`，调用会在 Writer 派发前失败关闭。Variables、Button 与 Icon 建库链路已有协议、Plan、Plugin Writer、幂等与审计，但尚未作为公开 MCP Tool 交给 Agent 自主调用。
...
- Variables、Button／Icon Component Set 与两类真实 Instance 的确定性 Writer 及幂等恢复；
...
- Icon 真实审批、Figma Desktop 建库与 Instance 插入验收，Input 组件链路；
