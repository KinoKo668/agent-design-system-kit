...
项目已完成 M0 架构与风险冻结以及 M1 工程骨架，当前处于 M2 Registry 读取与查询阶段。`core` 已提供统一结果、错误、恢复动作、日志、脱敏、Design Brief Schema、基础 Token Schema、Button v1 Component Contract、Component Registry Schema、跨资产快照校验、组件搜索与唯一解析；`mcp-server` 已实现本地文件加载和内容摘要验证。Change Request、CLI、MCP 协议服务与正式 Writer 尚未实现。
